...
# from torchvision.transforms import GaussianBlur      #uncomment it before you run editing_previous_img()  function
...
def image_to_image_stable_diffusion(img,some_prompt) :
    answers2 = stability_api.generate(some_prompt,init_image=img,start_schedule=0.6,seed=123467458,steps=30,cfg_scale=8.0,width=512,height=512,sampler=generation.SAMPLER_K_DPMPP_2M)
    for resp in answers2:
        for artifact in resp.artifacts:
            if artifact.finish_reason == generation.FILTER:
                warnings.warn(
            ...
            if artifact.type == generation.ARTIFACT_IMAGE:
                global img2
                img2 = Image.open(io.BytesIO(artifact.binary))
                img2.save(str(artifact.seed)+ "-img2img.png") 
...
some_prompt = input("Enter the text you want to edit the previous image for: ")
image_to_image_stable_diffusion(imgs,some_prompt)
